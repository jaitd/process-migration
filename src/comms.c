...
static void *publisher_routine(void *context) {
	...
	syslog(LOG_INFO, "Publisher Daemon Started");
	...
	syslog(LOG_INFO, "Socket bound to port 5555.");
	...
	while(1) {
		...
		info.nop = mynode->np;
		printf("Sent data: load: %0.2f nop: %d\n",mynode->load,mynode->np);
...
static void *subscriber_routine(void *context) {
	...
	syslog(LOG_INFO, "Subscriber Daemon Started");
	zmq_connect(sub, "tcp://192.168.1.14:5555");
	syslog(LOG_INFO, "Connected to 192.168.1.14 at port 5555");
...
int main()
{
	...
	openlog("PM_Communications Daemon:", LOG_CONS, LOG_LOCAL1);
