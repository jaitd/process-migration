...
if [ ! -z "$DMTCP_RESTART_DIR" ]; then
  new_ckpt_names=""
  names=" /home/jait/Downloads/dmtcp-1.2.1/test/ckpt_dmtcp1_e2fffbd-4177-4dfe63d2.dmtcp"
  for tmp in $names; do
...
if [ ! -z "$maybebatch" ]; then
  if [ ! -z "$DMTCP_RESTART_DIR" ]; then
    ...
    exec $DMTCP_RESTART $maybebatch $maybejoin --interval "$checkpoint_interval"\
        /home/jait/Downloads/dmtcp-1.2.1/test/ckpt_dmtcp1_e2fffbd-4177-4dfe63d2.dmtcp
  fi
else
  if [ ! -z "$DMTCP_RESTART_DIR" ]; then
