...
if [ ! -z "$DMTCP_RESTART_DIR" ]; then
  new_ckpt_names=""
  names=" /home/deep/work/dmtcp-1.2.1/test/ckpt_dmtcp2_7e0cf3d7-2099-4e04e25e.dmtcp"
  for tmp in $names; do
...
if [ ! -z "$maybebatch" ]; then
  if [ ! -z "$DMTCP_RESTART_DIR" ]; then
    ...
    exec $DMTCP_RESTART $maybebatch $maybejoin --interval "$checkpoint_interval"\
        /home/deep/work/dmtcp-1.2.1/test/ckpt_dmtcp2_7e0cf3d7-2099-4e04e25e.dmtcp
  fi
else
  if [ ! -z "$DMTCP_RESTART_DIR" ]; then
