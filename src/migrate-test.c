...
#define DMTCP_COORD_HOST "192.168.1.29"
...
{
	char command[256];
	sprintf(command, "dmtcp_command --host %s --port %s --quiet c", DMTCP_COORD_HOST, DMTCP_COORD_PORT);
	system(command);
}
...
{	
	...
	while(1) {
		...
		if(load > uthresh) { 
			...
			pthread_mutex_lock(&flagmutex);
				if(thresh_flag == 0) {
					fprintf(stderr,"PROCESS(S) WILL NOT BE MIGRATED AS ALL NODES ARE OVERLOADED\n");
					pthread_mutex_unlock(&flagmutex);
					continue;
				}
			...
			if(iload > iuthresh) {
				...
				sprintf(cmd,"xterm -e ./dmtcp_restart -h %s -p %s -q %s", DMTCP_COORD_HOST, DMTCP_COORD_PORT, ckpt);
